-- Customer, City, Rental, Category, Payment
use sakila;
...
-- main query
select sum(p.amount) as Total_money_spent, r.rental_id, r.rental_date, cu.customer_id, cu.first_name, cu.last_name, ci.city, count(fc.category_id) as 'Count', fc.category_id, c.name from payment as p
join rental as r on p.customer_id = r.customer_id
join customer as cu on cu.customer_id = r.customer_id
join address as a on a.address_id = cu.address_id
join city as ci on ci.city_id = a.city_id
...
SELECT concat (first_name,' ',last_name) as 'FullName', count(name), name from payment as p
join rental as r on p.customer_id = r.customer_id
join customer as cu on cu.customer_id = r.customer_id
join address as a on a.address_id = cu.address_id
join city as ci on ci.city_id = a.city_id
...
join film_category fc on inv.film_id=fc.film_id
join category c on fc.category_id = c.category_id
GROUP by first_name,last_name,'FullName', name
ORDER by first_name,last_name, 'FullName';
;

-- subquery
SELECT concat (first_name,' ',last_name) as 'FullName', count(name), name
...
SELECT customer_table.customer_id, city, category, number_of_films_rented, total_money_spent, table_query4.films_rented_may_2005, table_query5.question
FROM query1 as table_query1 
...
select distinct substr(rental_date,6,2) from rental;
